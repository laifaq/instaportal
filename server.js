...
app.get('/api/admin/stats', isAuthenticated, isAdmin, async (req, res) => {
    try {
        ...
        const totalPrompts = (await db.prepare('SELECT COUNT(*) as count FROM history').get()).count;
        const promptBreakdown = await db.prepare('SELECT type, COUNT(*) as count FROM history GROUP BY type').all();
        const recentUsers = await db.prepare('SELECT username, display_name, created_at FROM users ORDER BY created_at DESC LIMIT 5').all();

        res.json({ stats: { totalUsers, totalPrompts, promptBreakdown }, recentUsers });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
app.post('/api/admin/invite', isAuthenticated, isAdmin, async (req, res) => {
    const { email, display_name } = req.body;
    const token = crypto.randomBytes(32).toString('hex');
    const expires = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();

    try {
        await db.prepare('INSERT INTO invited_users (email, display_name, invited_by, setup_token, token_expires) VALUES (?, ?, ?, ?, ?)').run(
            email, display_name || null, req.session.userId, token, expires
        );
        const setupUrl = `${req.protocol}://${req.get('host')}/set-password.html?token=${token}`;
        res.json({ message: 'Invited', setupUrl, token, email });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});
...
// ==========================================
//  AI PROXY (Passthrough)
// ==========================================

app.post('/api/ai/enhance-prompt', isAuthenticated, async (req, res) => {
    const { prompt } = req.body;
    const user = await db.prepare('SELECT gemini_api_key FROM users WHERE id = ?').get(req.session.userId);
    if (!user?.gemini_api_key) return res.status(400).json({ error: 'Gemini API key missing' });
    ...
    try {
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                contents: [{ parts: [{ text: `Tingkatkan prompt ini: ${prompt}` }] }]
            })
        });
        const data = await response.json();
        res.json({ enhanced: data.candidates?.[0]?.content?.parts?.[0]?.text });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
});

// Route catch-all to serve index.html
app.get('*', (req, res) => {
